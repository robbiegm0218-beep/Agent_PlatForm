...
日常配置优先在“知识库 → 配置中心”完成，并按影响范围选择角色：普通用户维护自己的检索和上传默认值；知识库管理员维护处理预设；平台管理员维护检索候选、向量任务和历史迁移。处理预设保存为新修订，只影响后续上传或显式重新处理；检索候选必须先评测再发布；迁移同一时间只允许一个未结束批次。

高风险操作前先确认页面展示的影响数量和回滚目标：

1. 候选发布前确认父版本等于当前活动版本，固定集与混合门禁均通过。
2. 向量全库重建前确认当前资料数量；Provider 未配置时不要尝试开启任务。
3. 历史迁移按“暂存 → Shadow → 25% → 100%”推进；出现失败时只重试失败项，不新建第二批。
4. 灰度异常时立即使用页面回滚恢复迁移前版本；文档数量和 ACL 指纹必须保持一致。

Embedding 地址、凭证、模型、维度和超时只修改 `.env` 或进程环境，修改后重启本地源码服务。上传/解压/页数/字符限制、ACL、路径边界和 FTS 字段权重在配置中心只读，不能通过数据库或浏览器绕过。

发布前执行 `scripts/check-knowledge-configuration-release.sh`。该门禁不更新 Docker；是否构建镜像由实例所有者在本地灰度通过后单独决定。
