Create Codex pet row `running-right` for `hoops`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing compact chibi basketball character exactly: orange basketball body, white headband, tiny expressive face, white sneakers, black outline, pixel-art sprite style. Upgrade only with natural look directions.. Preserve silhouette, face, palette, material, proportions, markings, and props.
