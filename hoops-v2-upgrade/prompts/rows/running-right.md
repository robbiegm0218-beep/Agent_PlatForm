Create one horizontal animation strip for Codex pet `hoops`, state `running-right`.
...
Identity: same pet in every frame: Preserve the existing compact chibi basketball character exactly: orange basketball body, white headband, tiny expressive face, white sneakers, black outline, pixel-art sprite style. Upgrade only with natural look directions.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `pixel`: Pixel-art-adjacent digital mascot with a chunky silhouette, simple dark outline, limited palette, flat cel shading, and visible stepped edges. User style notes: Existing approved 8x9 sprite atlas is canonical; retain its identity and proportions..
