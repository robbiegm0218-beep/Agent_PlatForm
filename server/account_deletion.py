    ...
    with sqlite3.connect(database) as conn:
        ...
        for user_id in due_account_ids(conn, current_ns):
            ...
            if document_ids:
                ...
                conn.execute(f"DELETE FROM knowledge_chunks_fts WHERE document_id IN ({marks})", document_ids)
                ...
                conn.execute(f"DELETE FROM knowledge_document_blocks WHERE document_id IN ({marks})", document_ids)
                conn.execute(f"DELETE FROM knowledge_chunk_embeddings WHERE document_id IN ({marks})", document_ids)
                conn.execute(f"DELETE FROM knowledge_embedding_jobs WHERE document_id IN ({marks})", document_ids)
                conn.execute(f"DELETE FROM knowledge_chunks WHERE document_id IN ({marks})", document_ids)
                conn.execute(f"DELETE FROM knowledge_chunk_versions WHERE document_id IN ({marks})", document_ids)
                conn.execute(f"DELETE FROM knowledge_migration_items WHERE document_id IN ({marks})", document_ids)
                conn.execute(f"DELETE FROM citation_feedback_items WHERE document_id IN ({marks})", document_ids)
                conn.execute(f"DELETE FROM knowledge_pipeline_events WHERE ingestion_run_id IN (SELECT id FROM knowledge_ingestion_runs WHERE document_id IN ({marks}))", document_ids)
                conn.execute(f"DELETE FROM knowledge_ingestion_runs WHERE document_id IN ({marks})", document_ids)
                conn.execute(f"DELETE FROM knowledge_documents WHERE id IN ({marks})", document_ids)
            conn.execute("DELETE FROM knowledge_pipeline_events WHERE ingestion_run_id IN (SELECT id FROM knowledge_ingestion_runs WHERE user_id = ?)", (user_id,))
            conn.execute("DELETE FROM knowledge_ingestion_runs WHERE user_id = ?", (user_id,))
            ...
            conn.execute("DELETE FROM knowledge_migration_shadow_diffs WHERE actor_user_id = ?", (user_id,))
            conn.execute("DELETE FROM knowledge_migration_shadow_diffs WHERE batch_id IN (SELECT id FROM knowledge_migration_batches WHERE actor_user_id = ?)", (user_id,))
