...
| 面向用户的能力 | 底层保障 |
| --- | --- |
| 空间、任务、对话历史与 Markdown 回答渲染 | 空间可独立展开、排序、管理任务与查看空间产物；任务可按空间归属或独立存在；空间成员使用本地持久化邀请记录；SQLite 持久化、PBKDF2-SHA256 密码散列、会话过期与按用户限流 |
...
| Markdown、Excel 文件产物 | 仅写入 `data/artifacts/` 受控目录；创建前必须确认；路径、类型与审计受限；可在所属空间聚合查看 |
| 运行详情、步骤、工具调用与取消 | Run 状态机、事件序号和版本、服务重启时遗留运行自动收敛 |
| 平台状态、工作区文件名检索、可选网页检索 | 可解释的工具意图路由；优先 Tavily MCP、失败回退 REST；来源进入 Run 审计与对话右侧面板 |
